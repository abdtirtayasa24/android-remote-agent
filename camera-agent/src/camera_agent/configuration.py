...
class AgentConfig:
    ...
    def from_file(cls, path: Path) -> AgentConfig:
        ...
        capture_interval_seconds = _optional_integer(
            raw, "capture_interval_seconds", 60
        )
        ...
        if (
            not isinstance(output_directory_value, str)
            or not output_directory_value.strip()
        ):
        ...
        if not 5 <= capture_timeout_seconds <= 120:
            raise ConfigurationError(
                "capture_timeout_seconds must be between 5 and 120"
            )
